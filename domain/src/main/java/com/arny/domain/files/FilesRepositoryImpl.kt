...
class FilesRepositoryImpl @Inject constructor(
    ...
    fun writeToFile(filename: String, data: String) {
        ...
        try {
            ...
            val contentResolver = resourcesProvider.getContentResolver()
            val uri = contentResolver.insert(MediaStore.Files.getContentUri("external"), values)
            val os: OutputStream? = uri?.let { contentResolver.openOutputStream(it,"wt") }
    ...
    override fun saveExcelFile(dbFlights: List<Flight>, exportfilePath: String?): String? {
        var row: Row
        val wb = HSSFWorkbook()
        var c: Cell
        ...
        val file = File(getDefaultFilePath(resourcesProvider.provideContext(), exportfilePath))
        val success: Boolean = writeXlsFile(file, wb)
        if (success) {
            return file.path
        }
        return null
    ...
    private fun getDefaultFilePath(context: Context, exportfilePath: String?): String {
        val workDir = exportfilePath ?: FileUtils.getWorkDir(context)
        return workDir + File.separator + CONSTS.FILES.EXEL_FILE_NAME
    }
}